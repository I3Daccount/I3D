...
public:
	...
	void process_face(unsigned vcount, int *vertices, int *texcoords, int *normals)
	{
		...
		mesh.faces.push_back(idx_type(mesh.position_indices.size())); // face vector is also per vertex feature 
	...
	/// modi: yzy, 17.01.2021
	bool write_obj(const std::string& file_name) {

		std::string fn_str = cgv::utils::file::get_file_name(cgv::utils::file::drop_extension(file_name));
		std::string fn_mtl = cgv::utils::file::drop_extension(file_name) + ".mtl";

		std::ofstream os(file_name);
		if (os.fail())
			return false;
		std::ofstream os_mtl(fn_mtl);
		if (os_mtl.fail())
			return false;

		os << "mtllib " + fn_str + ".mtl" << std::endl;
		for (const auto& p : mesh.positions)
			os << "v " << p << std::endl;
		for (const auto& t : mesh.tex_coords)
			os << "vt " << t << std::endl;
		for (const auto& n : mesh.normals)
			os << "vn " << n << std::endl;
		// can not read color directly 
		/*for (const auto& c : )
			os << "vc " << c << std::endl;*/
		for (int i = 0; i < mesh.get_nr_colors(); i++) {
		...
		bool nmls = mesh.position_indices.size() == mesh.normal_indices.size();

		os << "\nusemtl material0" << std::endl;
		for (idx_type fi = 0; fi < mesh.faces.size(); ++fi) {
			os << "f";
			for (idx_type ci = mesh.begin_corner(fi); ci < mesh.end_corner(fi); ++ci) {
				os << " " << mesh.position_indices[ci] + 1;
				if (!nmls && !tcs)
					continue;
				os << "/";
				if (tcs)
					os << mesh.tex_coord_indices[ci] + 1;
				if (nmls)
					os << "/" << mesh.normal_indices[ci] + 1;
			}
			os << "\n";
		}
...
/// write simple mesh with material
template <typename T>
bool simple_mesh<T>::write_with_materials(const std::string& file_name)
{
	simple_mesh_obj_reader<T> reader(*this);
	return reader.write_obj(file_name);
	//obj_loader_generic<T> writer(*this);
		//std::ofstream os(file_name);
		//if (os.fail())
		//	return false;

		////os << "mtllib capsule.mtl" << std::endl;
...
//
template <typename T>
void simple_mesh<T>::compute_ray_mesh_intersections(vec3 ray)
{
...
//
template <typename T> 
void simple_mesh<T>::randomize_texcoordi()
{
	if (tex_coords.size())
		tex_coords.clear();
