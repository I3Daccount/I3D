...
class CGV_API vr_view_interactor : public stereo_view_interactor, public vr::vr_calibration_base
{
	...
	*/
	...
	/// path to calibration filezhizhi
...
public:
	...
	float get_tracking_rotation() { return tracking_rotation; } 
	void set_tracking_origin(vec3 ori) { 
		tracking_origin = ori;
		calibrate_driver();
		///this->update_driver_calibration(get_current_vr_kit()->get_driver());
		//vr::vr_kit* kit_ptr = get_current_vr_kit();
		//if (!kit_ptr)
		//	return;
		//kit_ptr->get_driver(); 
		//vec3 current_posi = reinterpret_cast<const vec3&>(kit_states.at(0).controller[0].pose[9]); 
		/*kit_states.at(0).controller[0].pose[9] = ori.x();
		kit_states.at(0).controller[0].pose[10] = ori.y();
		kit_states.at(0).controller[0].pose[11] = ori.z();*/

		//kit_states.at(0).hmd.pose[9] = ori.x();
		//kit_states.at(0).hmd.pose[10] = ori.y();
		//kit_states.at(0).hmd.pose[11] = ori.z();
	} 
	vec3 get_tracking_origin() { return tracking_origin; }
	//@}
private:
	mat34 start_pose;
public:
