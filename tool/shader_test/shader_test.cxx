...
bool convert_to_string(const std::string& in_fn, const std::string& out_fn, bool skip_comments = true)
{
	...
	if (os.fail())
		return false;
	//int max_chunk_size = 200;
	//int counter = 0;
	//// encode in base64 if this a cgv option
	//std::string tmp = "";
	////if (cgv::utils::has_option("ENCODE_SHADER_BASE64")) {
	//tmp += std::string("§");
	//int count = content.size() / max_chunk_size + 1;
	//int last_few_string = content.size() % max_chunk_size;
	//for (int i = 0; i < count; i++) {
	//	std::string t_content = content.substr(i * max_chunk_size, i * max_chunk_size + (i == count-1)? last_few_string: max_chunk_size);
	//	tmp+=cgv::utils::encode_base64(t_content);
	//}
	////}
	//content = tmp;
