	...
	namespace pointcloud {
		...
		void ICP::reg_icp(Mat& rotation_mat, Dir& translation_vec, point_cloud& S, point_cloud& Q, cgv::pointcloud::ICP::Sampling_Type icp_filter_type, cgv::render::context* ctx) {
			...
			for (int iter = 0; iter < maxIterations && abs(cost) > eps; iter++)
			{
				...
				cgv::math::mat<float> R(3, 3, &rotation_update_mat(0, 0));
				if (cgv::math::det(R) < 0) {
					...
					fS(1, 1) = 1;
					fS(2, 2) = -1;
					rotation_update_mat = fU * fS * cgv::math::transpose(fV);
				}
				...
				if (min >= abs(cost)) {
					...
					sourceCloud->rotate(cgv::math::quaternion<float>(rotation_update_mat));
					sourceCloud->translate(translation_update_vec);
